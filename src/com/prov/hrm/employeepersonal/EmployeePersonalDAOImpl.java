...
public class EmployeePersonalDAOImpl implements EmployeePersonalDAO
{
	...
	public int addEmpPersonal(EmployeePersonal employeepersonal)throws HibernateException,ConstraintViolationException {
		...
		try{
			...
			criteria.add(Restrictions.eq("employee",new Employee(employeepersonal.getEmployee().getEmployeeId())));
			criteria.add(Restrictions.eq("organizationId",employeepersonal.getOrganizationId()));
			...
			status=1;
				...
				Iterator<EmployeePersonal> iterator= criteria.list().iterator();
				while(iterator.hasNext()){
				EmployeePersonal emppersonal= iterator.next();
				emppersonal.setDeleteFlag(false);
				emppersonal.setInsertDate(date.toString());
				emppersonal.setUpdateDate(date.toString());
				Date fromDate = indate.parse(employeepersonal.getDateOfBirth());
				emppersonal.setDateOfBirth(outdate.format(fromDate));
				session.update(emppersonal);
				status=1;
				}
			}
