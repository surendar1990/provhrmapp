...
import com.prov.hrm.holidaylist.HolidayListDAOImpl;
...
import com.prov.hrm.leaveconfiguration.LeaveConfigurationDAOImpl;
...
import com.prov.hrm.leavetypescheme.LeaveTypeSchemeDAOImpl;
...
public class ProvHrmController {
	...
	HolidayListDAOImpl holidaylistdao=null;
	...
	LeaveTypeSchemeDAOImpl leavetypeschemedao=null;
	...
	LeaveConfigurationDAOImpl leaveconfigurationdao=null;
	...
	/*
	 * LeaveType table:tblleaveconfiguration
	 */
	// Get table:LeaveType by id
			@RequestMapping(value = "LeaveConfiguration/{leaveconfigurationId}", method = RequestMethod.GET)
			public ResponseEntity<LeaveConfiguration> getLeaveConfigurationsById(@PathVariable int leaveconfigurationId) {
				try {
					leaveconfigurationdao = new LeaveConfigurationDAOImpl();
					LeaveConfiguration leaveconfiguration = leaveconfigurationdao.getLeaveConfigurationsById(leaveconfigurationId);
					if (leaveconfiguration != null) {
						return new ResponseEntity<LeaveConfiguration>(leaveconfiguration, HttpStatus.OK);
					} else {
						return new ResponseEntity<LeaveConfiguration>(leaveconfiguration,HttpStatus.NO_CONTENT);
					}
	...
	@RequestMapping(value = "/LeaveConfiguration", method = RequestMethod.GET)
	public ResponseEntity<List<LeaveConfiguration>> getAllLeaveConfigurations(
			@RequestHeader int data) {
		try {
			leaveconfigurationdao = new LeaveConfigurationDAOImpl();
			List<LeaveConfiguration> leaveconfiguration = 			leaveconfigurationdao
					.getAllLeaveConfigurations(data);
			return new ResponseEntity<List<LeaveConfiguration>>(leaveconfiguration, HttpStatus.OK);
		} catch (HibernateException he) {
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
		} catch (Exception e) {
			e.printStackTrace();
	...
	@RequestMapping(value = "/LeaveConfiguration", method = RequestMethod.POST, headers = "Accept=application/json")
	public ResponseEntity<String> addLeaveConfiguration(@RequestBody String leaveconfiguration) {
		responseHeader.set("Content-type", "text/plain");
		try {
			LeaveConfiguration leaveconfig = (LeaveConfiguration) convertJsonToObject(leaveconfiguration,
					LeaveConfiguration.class);
			leaveconfigurationdao = new LeaveConfigurationDAOImpl();	
			int status = leaveconfigurationdao.addLeaveConfiguration(leaveconfig);
			if (status == 0) {
				return new ResponseEntity<String>("Error in inserting data",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else if (status == 2) {
				return new ResponseEntity<String>(
						"LeaveType name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			}else if (status == 3) {
				return new ResponseEntity<String>(
						"From Date Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			}
			else {
				return new ResponseEntity<String>("Inserted Succesfully",
	...
	@RequestMapping(value = "/LeaveConfiguration/{leaveconfigurationId}", method = RequestMethod.DELETE)
	public ResponseEntity<String> deleteLeaveConfiguration(@PathVariable int leaveconfigurationId) {
		responseHeader.set("Content-type", "text/plain");
		try {
			leaveconfigurationdao = new LeaveConfigurationDAOImpl();
			int status = leaveconfigurationdao.deleteLeaveConfiguration(leaveconfigurationId);
			if (status == 0) {
				return new ResponseEntity<String>("Error in deleting data",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else if (status == 2) {
				return new ResponseEntity<String>(
						"Leave Type is Already Refered", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	@RequestMapping(value = "/LeaveConfiguration/{leaveconfigurationId}", method = RequestMethod.PUT, headers = "content-type=application/json")
	public ResponseEntity<String> updateLeaveConfiguration(
			@RequestBody String updateleaveconfig, @PathVariable int leaveconfigurationId)
			throws Exception {
		responseHeader.set("Content-type", "text/plain");
		try {
			LeaveConfiguration leaveconfig = (LeaveConfiguration) convertJsonToObject(
					updateleaveconfig, LeaveConfiguration.class);
			leaveconfigurationdao = new LeaveConfigurationDAOImpl();
			leaveconfig.setLeaveconfigurationId(leaveconfigurationId);
			int status = leaveconfigurationdao.updateLeaveConfiguration(leaveconfig);
			if (status == 0) {
				return new ResponseEntity<String>("Error in updating data",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				if (status == 5) {
					return new ResponseEntity<String>(
							"LeaveType Name Already Exists",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
							HttpStatus.OK);
				}
			}
		} catch (ConstraintViolationException ce) {
			return new ResponseEntity<String>(
					"LeaveType in between dates Already Exists",
					responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public ResponseEntity<String> addBank(@RequestBody String addBank) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("Bank name Duplicate Entry",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Inserted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> deleteBank(@PathVariable int bankId) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("Bank Already Refered",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateBank(@RequestBody String updateBank,
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>("Bank Already Exists",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<String> addCountry(@RequestBody String addCountry) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Country name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Inserted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> deleteCountry(@PathVariable int countryId) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("Country Already Refered",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateCountry(
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>("Country Already Exists",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<String> addDepartment(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Departmant name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
	...
	public ResponseEntity<String> deleteDepartment(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Department is Already Refered", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateDepartment(
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>(
							"Department Already Exists", responseHeader,
							HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<String> addDesignation(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Designation name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Data inserted Successfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> deleteDesignation(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Designation is Already Refered", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateDesignation(
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>(
							"Designation Already Exists", responseHeader,
							HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<String> addEducationlevel(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"EducationLevel name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Data inserted Successfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> deleteEducationlevel(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Education Level is Already Refered", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateEducationlevel(
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>(
							"Education Level Already Exists", responseHeader,
							HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<String> addEmployeeBank(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("Already Exist",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			}
			else {
				return new ResponseEntity<String>("Inserted Succesfully",
	...
	// GET A RECORD FROM EMPLOYEE LEAVE TABLE USING employeeId
	@RequestMapping(value = "EmployeeLeaveApproval/{employeeId}", method = RequestMethod.GET)
	public ResponseEntity<List<EmployeeLeave>> getEmployeeLeaveForApproval(
			@PathVariable int employeeId, @RequestHeader int data) {
		try {
			employeeleavedao = new EmployeeLeaveDAOImpl();
			List<EmployeeLeave> employeeleave = employeeleavedao
					.getAllEmployeeLeaveForEmployeeApproval(employeeId, data);
			if (employeeleave != null) {
				return new ResponseEntity<List<EmployeeLeave>>(employeeleave,
						HttpStatus.OK);
			} else {
	...
	@RequestMapping(value = "EmployeeLeaveStatus/{fromDt}/{toDt}/{employeeId}", method = RequestMethod.GET)
	public ResponseEntity<List<EmployeeLeave>> getEmployeeLeavetByDate(
			@PathVariable int employeeId, @RequestHeader int data,@PathVariable String fromDt,@PathVariable String toDt) {
		try {
			employeeleavedao = new EmployeeLeaveDAOImpl();
			List<EmployeeLeave> employeeleave = employeeleavedao
					.getEmployeeLeaveByDate(employeeId, data,fromDt,toDt);
			if (employeeleave != null) {
				return new ResponseEntity<List<EmployeeLeave>>(employeeleave,
						HttpStatus.OK);
			} else {
	...
	public ResponseEntity<String> addEmployeeLeave(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("Leave Has Been Already Applied On Same Date",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Inserted Succesfully",
						responseHeader, HttpStatus.OK);
		...
		try {
			...
			if (employeeleaveeligibility != null) {
				for (Object object1 : employeeleaveeligibility) {
					...
					from_date = (String) array[3];
					to_date = (String) array[4];
					if (from_date != null && to_date != null) {
						java.util.Date from_date1 = sdinput.parse(from_date);
						java.util.Date to_date1 = sdinput.parse(to_date);
						fd = sdfOut.format(from_date1);
						td = sdfOut.format(to_date1);
					}
					if (array[5] != null) {
						eligibilitydays = (Integer) array[5];
					} else {
						eligibilitydays = new Integer(0);
					}
	...
	// Get holiday by HolidayListName
	@RequestMapping(value = "Holiday/{holidayListId}/{year}", method = RequestMethod.GET)
	public ResponseEntity<List<Holiday>> getHolidayByListName(@PathVariable int holidayListId,
			@RequestHeader int data,@PathVariable int year) {
		try {
			holidaydao = new HolidayDAOImpl();
			List<Holiday> holiday = holidaydao.getHolidayByListName(holidayListId, data,year);
			if (holiday != null) {
			...
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
				...
				@RequestMapping(value = "/Holiday/{year}", method = RequestMethod.GET)
				public ResponseEntity<List<Holiday>> getAllHolidayTest(@RequestHeader int data,
						@PathVariable int year) {
					try {
						holidaydao = new HolidayDAOImpl();
						List<Holiday> holiday = holidaydao.getAllHoliday(data,year);
						return new ResponseEntity<List<Holiday>>(holiday, HttpStatus.OK);
					} catch (HibernateException he) {
						return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
					} catch (Exception e) {
		...
		public ResponseEntity<String> addHolidaytest(@RequestBody List<Holiday> addHoliday) {
			responseHeader.set("Content-type", "text/plain");
			try {

				
				holidaydao = new HolidayDAOImpl();
				long status = holidaydao.addHoliday(addHoliday);
				if (status ==100) {
					return new ResponseEntity<String>("Error in inserting data",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				} else if (status == 101) {
					...
					Holiday holiday= addHoliday.get((int) status-1);
					return new ResponseEntity<String>(
							holiday.getHolidayName()+" Already Exist", responseHeader,
							HttpStatus.INTERNAL_SERVER_ERROR);
				}
			} catch (ConstraintViolationException ce) {
				ce.printStackTrace();
		...
		public ResponseEntity<String> deleteHoliday(@PathVariable int holidayId) {
			...
			try {
				...
				{
					return new ResponseEntity<String>("Employee Already Applied Leave Using THis Holiday",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);	
				}else {
					return new ResponseEntity<String>("Error in deleting data",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
		...
		public ResponseEntity<String> updateHoliday(
			...
			try {
				...
				if (status == 0) {
					...
					if (status == 5) {
						return new ResponseEntity<String>(
								"Already Date Is Assigned To Employee For Leave Cant Edit",
								responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
					} else if (status == 6) {
						return new ResponseEntity<String>(
								"Holiday Name Or Date Duplicate Entry",
								responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
					} else {
						return new ResponseEntity<String>(
								"Data Updated Successfully", responseHeader,
		...
		// Get all holidayList
				...
				public ResponseEntity<List<HolidayList>> getAllHolidayList(@RequestHeader int data) {
					try {
						holidaylistdao=new HolidayListDAOImpl();
						List <HolidayList> holidaylist = holidaylistdao.getAllHolidayList(data);
						if (holidaylist != null) {
							return new ResponseEntity<List<HolidayList>>(holidaylist, HttpStatus.OK);
						} else {
							return new ResponseEntity<List<HolidayList>>(HttpStatus.NO_CONTENT);
						}
				...
				@RequestMapping(value = "/HolidayList/{holidaylistId}", method = RequestMethod.GET)
				public ResponseEntity<HolidayList> getAllHoliday(	@PathVariable int holidaylistId) {
					try {
						holidaylistdao = new HolidayListDAOImpl();
						HolidayList holidaylist = holidaylistdao.getHolidayListById(holidaylistId);
				...
				// Insert a record in holidaylist
				@RequestMapping(value = "/HolidayList", method = RequestMethod.POST, headers = "Accept=application/json")
				public ResponseEntity<String> addHolidayList(@RequestBody String addHolidaylist) {
					responseHeader.set("Content-type", "text/plain");
					try {

						HolidayList holidaylist = (HolidayList) convertJsonToObject(addHolidaylist,
								HolidayList.class);
						holidaylistdao=new HolidayListDAOImpl();
						int status = holidaylistdao.addHolidayList(holidaylist);
						if (status == 0) {
							return new ResponseEntity<String>("Error in inserting data",
									responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
						} else if (status == 2) {
							return new ResponseEntity<String>(
									"Holiday List Name is Already Exist", responseHeader,
									HttpStatus.INTERNAL_SERVER_ERROR);
						} else {
							return new ResponseEntity<String>("Inserted Succesfully",
									responseHeader, HttpStatus.OK);
				...
				// Delete holiday by id
				@RequestMapping(value = "HolidayList/{holidaylistId}", method = RequestMethod.DELETE)
				public ResponseEntity<String> deleteHolidayList(@PathVariable int holidaylistId) {
					responseHeader.set("Content-type", "text/plain");
					try {
						holidaylistdao=new HolidayListDAOImpl();
						int status = holidaylistdao.deleteHolidayList(holidaylistId);
						if (status==1) {
							return new ResponseEntity<String>("Deleted Succesfully",
									responseHeader, HttpStatus.OK);
						}
						...
						{
							return new ResponseEntity<String>("Holiday List Has Been Already Assigned",
									responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);	
						}else {
							return new ResponseEntity<String>("Error in deleting data",
									responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				...
				// Update holiday by id
				@RequestMapping(value = "HolidayList/{holidaylistId}", method = RequestMethod.PUT, headers = "Accept=application/json")
				public ResponseEntity<String> updateHolidayList(
						@RequestBody String updateHolidaylist, @PathVariable int holidaylistId)
						throws Exception {
					responseHeader.set("Content-type", "text/plain");
					try {
						HolidayList holidaylist = (HolidayList) convertJsonToObject(updateHolidaylist,
								HolidayList.class);
						holidaylistdao=new HolidayListDAOImpl();
						holidaylist.setHolidayListId(holidaylistId);
						int status = holidaylistdao.updateHolidayList(holidaylist);
						if (status == 0) {
							return new ResponseEntity<String>("Error in updating data",
									responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
						} else if (status == 2) {
								return new ResponseEntity<String>(
										"Holiday List Name is Already Exist",
										responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
							} else {
								return new ResponseEntity<String>(
										"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<String> addIdtype(@RequestBody String addIdtype) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Idtype name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Inserted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> deleteIdtype(@PathVariable int idtypeId) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("Id Type is Already Refered",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateIdtype(
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>(
							"Id proof Already Exists", responseHeader,
							HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	// Get table:LeaveType by id
			...
			public ResponseEntity<LeaveType> getLeaveTypeById(@PathVariable int leavetypeId) {
				try {
	...
	/*
	 * LeaveType table:tblleavetypescheme
	 */
	
	
	// Get table:tblleavetypescheme by id
		@RequestMapping(value = "LeaveTypeScheme/{leavetypeschemeId}", method = RequestMethod.GET)
		public ResponseEntity<LeaveTypeScheme> getLeaveTypeSchemeById(@PathVariable int leavetypeschemeId) {
			try {
				leavetypeschemedao = new LeaveTypeSchemeDAOImpl();
				LeaveTypeScheme leavetypescheme = leavetypeschemedao.getLeaveTypeSchemeById(leavetypeschemeId);
				if (leavetypescheme != null) {
					return new ResponseEntity<LeaveTypeScheme>(leavetypescheme, HttpStatus.OK);
				} else {
					return new ResponseEntity<LeaveTypeScheme>(HttpStatus.NO_CONTENT);
				}
	...
	// Get all the record in leavetypescheme
	@RequestMapping(value = "/LeaveTypeScheme", method = RequestMethod.GET)
	public ResponseEntity<List<LeaveTypeScheme>> getAllLeaveTypeScheme(
			@RequestHeader int data) {
		try {
			leavetypeschemedao = new LeaveTypeSchemeDAOImpl();
			List<LeaveTypeScheme> leavetypescheme = leavetypeschemedao
					.getAllLeaveTypeScheme(data);
			return new ResponseEntity<List<LeaveTypeScheme>>(leavetypescheme, HttpStatus.OK);
		} catch (HibernateException he) {
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
		} catch (Exception e) {
			e.printStackTrace();
	...
	// Insert a record into LeaveTypeScheme
	@RequestMapping(value = "/LeaveTypeScheme", method = RequestMethod.POST, headers = "Accept=application/json")
	public ResponseEntity<String> addLeaveTypeScheme(@RequestBody String addLeaveTypeScheme) {
		responseHeader.set("Content-type", "text/plain");
		try {
			LeaveTypeScheme leavetypescheme = (LeaveTypeScheme) convertJsonToObject(addLeaveTypeScheme,
					LeaveTypeScheme.class);
			leavetypeschemedao = new LeaveTypeSchemeDAOImpl();
			int status = leavetypeschemedao.addLeaveTypeScheme(leavetypescheme);
			if (status == 0) {
				return new ResponseEntity<String>("Error in inserting data",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else if (status == 2) {
				return new ResponseEntity<String>(
						"LeaveType Scheme name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Inserted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	// Delete a record in LeaveTypeScheme
	@RequestMapping(value = "/LeaveTypeScheme/{leavetypeschemeId}", method = RequestMethod.DELETE)
	public ResponseEntity<String> deleteLeaveTypeScheme(@PathVariable int leavetypeschemeId) {
		responseHeader.set("Content-type", "text/plain");
		try {
			leavetypeschemedao = new LeaveTypeSchemeDAOImpl();
			int status = leavetypeschemedao.deleteLeaveTypeScheme(leavetypeschemeId);
			if (status == 0) {
				return new ResponseEntity<String>("Error in deleting data",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else if (status == 2) {
				return new ResponseEntity<String>(
						"Leave Type scheme is Already Refered", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	// Update a record in LeaveTypeScheme
	@RequestMapping(value = "/LeaveTypeScheme/{leaveTypeSchemeId}", method = RequestMethod.PUT, headers = "content-type=application/json")
	public ResponseEntity<String> updateLeaveTypeScheme(
			@RequestBody String updateleavetypescheme, @PathVariable int leaveTypeSchemeId)
			throws Exception {
		responseHeader.set("Content-type", "text/plain");
		try {
			LeaveTypeScheme leavetypescheme = (LeaveTypeScheme) convertJsonToObject(
					updateleavetypescheme, LeaveTypeScheme.class);
			leavetypeschemedao = new LeaveTypeSchemeDAOImpl();
			leavetypescheme.setLeaveTypeSchemeId(leaveTypeSchemeId);
			int status = leavetypeschemedao.updateLeaveTypeScheme(leavetypescheme);
			if (status == 0) {
				return new ResponseEntity<String>("Error in updating data",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				if (status == 5) {
					return new ResponseEntity<String>(
							"LeaveTypeScheme Name Already Exists",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<Login> Authentication(@RequestBody String checklogin) {
		try {
			logindao = new LoginDAOImpl();
			Login login = (Login) convertJsonToObject(checklogin, Login.class);
			Login loginDetails = logindao.Authentication(login);
			System.out.println(loginDetails);
			if (loginDetails != null) {
				return new ResponseEntity<Login>(loginDetails, HttpStatus.OK);
			} else {
	...
	public ResponseEntity<String> addOrganization(
		...
		try {
			...
			{
				return new ResponseEntity<String>("Organization email already exists",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					...
					return new ResponseEntity<String>(
							"Updated Successfully", responseHeader,
							HttpStatus.OK);
				}
			}
		} catch (ConstraintViolationException ce) {
	...
	public ResponseEntity<String> deleteOrganization(
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"Organization is Already Refered", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> addState(@RequestBody String addState) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("State name Duplicate Entry",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Inserted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateState(@RequestBody String updatestate,
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>("State Already Exists",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<String> deleteState(@PathVariable int stateId) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("State is Already Refered",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> addVisatype(@RequestBody String addVisa) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>(
						"VisaType name Duplicate Entry", responseHeader,
						HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Data inserted Successfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> deleteVisatype(@PathVariable int VisaId) {
		...
		try {
			...
			if (status == 0) {
				...
				return new ResponseEntity<String>("Visa is Already Refered",
						responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
			} else {
				return new ResponseEntity<String>("Deleted Succesfully",
						responseHeader, HttpStatus.OK);
	...
	public ResponseEntity<String> updateVisatype(
		...
		try {
			...
			if (status == 0) {
				...
				if (status == 5) {
					return new ResponseEntity<String>("Visa Already Exists",
							responseHeader, HttpStatus.INTERNAL_SERVER_ERROR);
				} else {
					return new ResponseEntity<String>(
							"Data Updated Successfully", responseHeader,
	...
	public ResponseEntity<List<LeaveCalculation>> LeaveCalculator(
			@RequestHeader int data, @PathVariable int employeeId) {
		
		LeaveCalculator leave = new LeaveCalculator();
		try {
			List<LeaveCalculation> leavecalculation = leave.LeaveCalculation(
					data, employeeId);
	...
	/*@RequestMapping(value = "/LeaveCalculatorEdit/{employeeleaveId}", method = RequestMethod.GET)
	public ResponseEntity<List<LeaveCalculation>> LeaveCalculatorEdit(
			@PathVariable int employeeleaveId,@PathVariable String accYear) {
		LeaveCalculator leave = new LeaveCalculator();
		try {
			List<LeaveCalculation> leavecalculation = leave
					.LeaveCalculatorEdit(employeeleaveId,accYear);
			return new ResponseEntity<List<LeaveCalculation>>(leavecalculation,
					HttpStatus.OK);
		}
